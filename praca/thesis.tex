...
\chapter{External specification}
  \section{Types of users and cockpits}
    ...
    \end{itemize}
    Each cockpit has certain role that is necessary to even display available actions. 
    ...
    User with role of a voter is able to vote for election, and see election results, after it's closed.
    In case of voting, user is be able to vote on a poll only once, without any exception.

    \subsection{Administrator cockpit}
    User with role of an administrator is able to create essential data---wards, polls, committees etc. as well as granting certain roles to certain users.
    Administrator is not be able to interfere in ward or committee administration, other than choosing its administrator.
    Moreover, administrator is the one that triggers polls results calculation.

    \subsection{Ward administrator cockpit}
    User with role of ward administrator is able to manage only his assigned ward. Under no circumstances he is able to manage other wards.
    In ward administrator cockpit he is able to enter protocol from ward for each poll during an election.

    \subsection{Committee administrator cockpit}
    User with role of committee administrator is able to manage only his assigned committee. Under no circumstances he is able to manage other committees.
    In committee administrator cockpit he is able to choose order of committee candidates in closed list. 
  ...
  \section{Installation procedure}
    ...
    In order to build project, we have to execute Maven install lifecycle \lstinline[language=bash]|mvn install| in APP\_ROOT directory.
  ...
  \section{Activation procedure}
    Users' roles definitions are loaded into database on first server startup if they don't exist in the database already.
    Same behavior with default admin user with password 'nimda'---it's in the database on first startup.
    To secure the application, we have to register proper user which is meant to be administrator, and execute following query in the database:
  ...
  \section{User manual}
    \subsection{Voter manual}
      Voter can access voting page at root endpoint of the application---\lstinline|HOST:PORT|.
      ...
      Second one is preference voting, in which voter can mark candidates by his choice of preference.
      1 for candidate of his first choice, 2 for his second favorite candidate and so on.

      /////preference vote screenshot/////

      Description provides user with specific instructions on how to vote on specific poll. If user doesn't follow the instructions, vote is invalidated, and voter cannot vote again.
